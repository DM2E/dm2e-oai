...
public class OaiDublinCoreRecord extends BaseXMLExporter {
	...
	//		baseName = baseName.replaceAll("[^a-zA-Z0-9]+", "");
			...
			// <dc:type>
			{
				...
				xml.writeCharacters(Dm2eUtils.lastUriSegment(resMap.getDcType()));
				// </dc:type>
				xml.writeEndElement(); // </dc:type>
			}

			// <dc:identifier linktype='rdf'>
			{
				// <dc:identifier>
			...
			// <dc:identifier linktype="isPartOf">
			{
				ResourceMap containingResourceMap = resMap.getContainingResourceMap();
				if (null != containingResourceMap) {
					// <dc:identifier>
					xml.writeStartElement(NS.DC.BASE, "identifier"); 
					xml.writeAttribute("linktype", "isPartOf");
					xml.writeCharacters(containingResourceMap.getProvidedCHO_Uri());
					// </dc:identifier>
					xml.writeEndElement(); // </dc:identifier>
				}
			}
			...
			for (String theProp : getChoDateProperties()) {
				...
				if (null == dateTime) {
					ResourceMap parentResourceMap = resMap.getContainingResourceMap();
					if (null != parentResourceMap)
						dateTime = parentResourceMap.getDateTimeForProp(parentResourceMap.getProvidedCHO_Resource(), theProp);
				}
				if (null != dateTime) {
					// <dc:date>
